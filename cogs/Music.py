...
class Music(commands.Cog, name="Music commands"):
    ...
    @commands.command(name="vc", brief="Join/Leave voice channel", description="", pass_context=True)
    async def vc(self, ctx, action = "join"):
    ...
    @commands.command(name="music", brief="Music player", description="", pass_context=True)
    async def musicplayer(self, ctx, action = "play"):
        if permissions.check(ctx.message.author.id):
            global vchan
            global vmstop
            try:
